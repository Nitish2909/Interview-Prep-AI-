      ...
      <div className="bg-white rounded-lg mb-4 overflow-hidden py-4 px-5 shadow-xl shadow-gray-100/70 border border-gray-100/60 group ">
        <div className="flex items-start justify-between cursor-pointer">
          <div className="flex items-start gap-3.5">
            ...
            <h3 className="text-xs md:text-[15px] font-semibold text-amber-800 mr-0 md:mr-20 " onClick={toggleExpand}>
              {question}
            </h3>
          </div>
          <div className="flex items-center justify-end ml-4 relative">
            ...
            >
              ...
              <button
                className="flex items-center gap-2 text-xs text-cyan-800 font-medium bg-cyan-50 px-3 py-1 mr-2 rounded text-nowrap border border-cyan-50 hover:bg-cyan-200 cursor-pointer"
                onClick={() => {
                  setIsExpanded(true);
                  oneLearnMore();
                }}
            ...
            <button className="text-gray-400 text-gray-500 cursor-pointer" onClick={toggleExpand}>
              <LuChevronDown
                size={20}
        ...
        <div className="overflow-hidden transition-all duration-300 ease-in-out " style={{ maxHeight: `${height}px` }}>
          <div className="mt-4 text-gray-700 bg-gray-50 px-5 py-3 rounded-lg" ref={contentRef}>{answer}</div>
        </div>
      </div>
    </>
  ); 
